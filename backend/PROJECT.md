...
Pydantic models (or a collection of them), rather than plain dicts, since it
...
convenient collection of all service classes, to avoid having to pass them
...
with table definitions, but rather contain fields that are needed for specific
...
which is achieved via [Alembic](https://alembic.sqlalchemy.org/). The migrations
...
Currently, authentication for both the `user` and `site` APIs uses JWT.  Tokens
are signed with a secret key that's autogenerated at the first service startup
and stored in the database (`"token_secret_key"` in the `config` table), so
that it can be used by any instance of the API service.

The bulk of the logic is implemented in `msm.jwt`, which defines a `JWT`
wrapper class for encoding and decoding tokens.

When the service first starts up, a random UUID is generated that identifies
the service (`"service_identifier"` in the `config` table). This is used as
value for the issuer (`"iss"`) field for JWTs, and matched during
authentication.

Similarly, authenticating entities (users or sites) are assigned a random
authentication ID on creation (again, a UUID) which is used as value for the
subject (`"sub"`) field in the token.

Regarding the payload of the tokens, aside from required fields, tokens include
a few extra fields:

- `"aud"`, which is standard but not required, is used to distinguish for which
  API the token is intended for. Supported values are defined in
