...
class AsyncIterator:
    """
    Class for mocking async iterators
    """
...
class TestTemporalService:
    ...
    async def test_client_success(
        ...
        _ = mocker.patch(
            "msm.service.temporal.Client.connect",
    ...
    async def test_schedule_create_success(
        ...
        from msm.service.temporal import MsmApiWorkflowParams

        scheduler_id = "test-scheduler"
        workflow = "TestWorkflow"
        workflow_id = "test-workflow-123"
        param = MsmApiWorkflowParams(
            msm_url="https://example.com", msm_jwt="test-jwt-token"
        )
        interval = 30
